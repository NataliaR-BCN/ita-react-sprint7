...
function Home() {
  ...
  useEffect(()=>{
    searchParams.set('nomProjecte', projectData[0].projectName);
    searchParams.set('nomClient', projectData[1].clientName);
    searchParams.set('paginaWeb', servicesSelected[0].webPage);
    searchParams.set('consultoriaSeo', servicesSelected[1].seoConsulting);
    searchParams.set('campanyaGoogleAds', servicesSelected[2].googleAdsCampaign);
    searchParams.set('numeroPagines', webPageNumbers.numPages);
    searchParams.set('numeroIdiomes', webPageLangs.numLangs);
  ...
  return (   
    ...
    <Columnes>
      ...
      <Calculadora inputHandler={inputChange} projectName={projectData[0].projectName} clientName={projectData[1].clientName} onChangeHandler={handleChange} webPageCheck={servicesSelected[0].webPage} 
        ...
        changeNumberUpPanell={increaseNumber} generarBudget={generarBudget} cleanData={cleanFormOldData} />
      </ColEsquerra>
      <ColDreta>
     
        { llistaPlena && <Llistat handleReiniciarOrdre={handleOrdreEntrada} 
                            handleOrdreAlfabetic={handleOrdreAlfabetic} handleOrdreData={handleOrdreData}
                            showLlistat ={showLlistat()} handleBuscador={handleBuscador} 
                            handleValueBuscador={buscarProjecte} /> }
