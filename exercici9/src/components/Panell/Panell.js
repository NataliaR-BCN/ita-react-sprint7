import BotoInfo from '../BotoInfo/BotoInfo';
import InputInteractiu from '../InputInteractiu/InputInteractiu';
import { PanellDiv, Fila, Label } from './Panell.styles';


const Panell = (props) => 

<PanellDiv>
 
        <Fila>
            <Label htmlFor="numPages">Número de pàgines</Label> 
            <InputInteractiu
                id={"numPages"} 
                name={"numPages"} 
                onChangeHandler={props.handle} 
                number={props.numPagesValue} 
                handleDown={props.changeNumberDown}
                handleUp={props.changeNumberUp}
            />
            <BotoInfo name={"numPages"} number={props.numPagesValue} />
        </Fila>
        <Fila>
            <Label htmlFor="numLangs">Número d'idiomes</Label> 
            <InputInteractiu 
                id={"numLangs"} 
                name={"numLangs"} 
                onChangeHandler={props.handle} 
                number={props.numLangsValue} 
                handleDown={props.changeNumberDown}
                handleUp={props.changeNumberUp}
            />
            <BotoInfo name={"numLangs"} number={props.numLangsValue} />
        </Fila>

</PanellDiv>;

export default Panell;